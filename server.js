...
var nodemailerFrom = "cyberboyarup@gmail.com";
var nodemailerObject ={
    ...
    secure: true,
    auth: {
        user: "cyberboyarup@gmail.com",
        pass: "kwlboxffdeawivgt"
...
http.listen(3000, function () {
    ...
    // connect with mongo DB server
        mongoClient.connect("mongodb+srv://arup:Arup@2018@cluster0.y6njcsm.mongodb.net/?retryWrites=true&w=majority", {
            ...
            if (user == null) {
                bcrypt.hash(password, 10, async function (error, hash) {
                    await database.collection("users").insertOne({
                        ...
                        var html = "Please verify your email address by clicking on the link below:<br><br><a href='"+ mainURL + "/VerifyEmail/" +email+"/"+ verification_token +"'>Confirm Email</a> <br><br>  Arup Mandal";
        ...
        app.get("/VerifyEmail/:email/:verification_token", async function (request, result) {
            ...
            var user = await database.collection("users").findOne({
                ...
                if (user == null) {
                    request.status = "error";
                    request.message = "Invalid verification link.";
                    result.render("Login", {
                        "request": request
                    });
                } else{
                    ...
                    request.status = "success";
                    request.message = "Email has been verified. You can login now.";
                    result.render("Login", {
                        "request": request
                    });
                }
        ...
        app.post("/SendRecoveryLink", async function (request, result) {
            ...
            var html = "Please reset your password by clicking on the link below:<br><br><a href='"+ mainURL + "/ResetPassword/" +email+"/"+ reset_token +"'>Reset Password</a> <br><br>  Arup Mandal";
            await transporter.sendMail({
                from: nodemailerFrom,
                to: email,
                ...
                request.status = "success";
                request.message = "Recovery link has been sent to your email.";
                result.render("ForgotPassword", {
                    "request": request
        ...
        app.get("/ResetPassword/:email/:reset_token", async function (request, result) {
            ...
            var user = await database.collection("users").findOne({
                ...
                if (user == null) {
                    request.status = "error";
                    request.message = "Invalid reset link.";
                    result.render("Error", {
                        "request": request
                    });
                    return false;
        ...
        app.post("/ResetPassword", async function (request, result) {
            ...
            var new_password = request.fields.password;
            ...
            var user = await database.collection("users").findOne({
                ...
                request.status = "success";
                request.message = "Password has been reset. You can login now.";
                result.render("Login", {
                    "request": request
                });
